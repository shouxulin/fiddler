...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < iter+warmup_iter; i++)
    {
        ...
        random_bf16(W_2[i], D * F, 0.02f, 44);

        W_3[i] = (uint16_t*)malloc(D * F * sizeof(uint16_t));
        random_bf16(W_3[i], D * F, 0.02f, 44);
    }
    ...
    {   
        ...
        // GEMM: gate = X @ W1^T
        gemm_rowmajor_bf16(handle, X_dev, (__nv_bfloat16*) W_1[i], gate, B, D, F);
        ...
        // GEMM: up = X @ W3^T
        gemm_rowmajor_bf16(handle, X_dev, (__nv_bfloat16*) W_3[i], up, B, D, F);
        ...
        // out = h @ W2ᵀ → [B, d_model]
        gemm_rowmajor_bf16(handle, h, (__nv_bfloat16*) W_2[i], out, B, F, D);
