...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < iter + warmup_iter; i++)
    {
        ...
        gemm_rowmajor_bf16(handle, d_A[i], (__nv_bfloat16*)B[i], d_C[i], batch_size, hidden_size, expert_size);
        // bool ok = verify_bf16_gemm_cpu_sample(d_A[i], (__nv_bfloat16*)B[i], d_C[i], batch_size, hidden_size, expert_size,
        //                                   /*num_samples=*/10,
